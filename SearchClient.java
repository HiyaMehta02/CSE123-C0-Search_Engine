...
public class SearchClient {
    ...
    // Behavior: Creates an inverted index mapping each token in the document
    // content to a set of documents containing that token. The method
    // processes tokens case-insensitively, meaning "Word" and "word"
    // are treated as the same token.
    // Exceptions: None.
    // Returns: A Map where keys are tokens (case-insensitive) and values are sets
    // of Media documents containing those tokens.
    ...
    public static Map<String, Set<Media>> createIndex(List<Media> docs) {
        ...
        for (Media doc : docs) {
            ...
            for (String token : content) {
                ...
                invertedIndex.get(token).add(doc);
            }
        }
        return invertedIndex;
    }

    // Behavior: Searches the inverted index for documents containing the specified
    // query token. The query is processed case-insensitively, meaning
    // "Word" and "word" are treated as the same.
    // Exceptions: None.
    // Returns: A Set of Media documents that contain the queried token. If the
    // query
    // is null, empty, or not found, an empty Set is returned.
    // Parameters: index - A Map where keys are tokens and values are sets of Media
    // documents containing those tokens.
    // query - The token to search for in the index.
